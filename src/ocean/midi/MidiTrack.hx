...
class MidiTrack
{
    public var msgList(get, set) : MessageList;
    public var trackChannel(get, set) : Int;
    public var trackPatch(get, set) : Int;

    /**
		* Track header tag
		*/
    private static inline var MTrk : Int = 0x4D54726B;  //[ 0x4D , 0x54 , 0x72 , 0x6B ];  
    ...
    /**
		 ...
		 */
    private function get_msgList() : MessageList
    {
        return _msgList;
    }
    ...
    /**
		...
		*/
    private function set_msgList(ml : MessageList) : MessageList
    {
        _msgList = ml;
    ...
    public function serialize(stream : ByteArray) : Void
    {
        ...
        var start : Int = stream.position;  //track data start point excluding MTrk and tracksize  
        var rawArray : Array<Dynamic> = new Array<Dynamic>();  //cache the raw items  
        ...
        //reads active item{
            ...
            if (item.kind == MidiEnum.META)
            {
                ...
                //big enough for keep the end-of-track at the end of list
                else
                {
                    
                    rawItem.index = index++;
                }  //index is utilized when stably sort rawitems.  
        ...
        _size = as3hx.Compat.parseInt(stream.position - start);
    ...
    //check MTrk
    {
        ...
        var end : Int = as3hx.Compat.parseInt(stream.position + _size);
        ...
        var queue : Array<Dynamic> = new Array<Dynamic>();  // not a queue, just a basket for picking unclosed note.  
        var guint : GreedyUINT = new GreedyUINT();
        var timeline : Int = 0;
        var byte : Int;  //read one byte  
        var char : Int;  //cache byte&0xF0  
        var channel : Int;
        var command : Int;
        //loop reading
    ...
    /**
		 ...
		 */
    private function get_trackChannel() : Int
    {
    ...
    /**
		 * Patch of this track.
		 */
    private function get_trackPatch() : Int
    {
        return _trackPatch;
    }
    ...
    /**
		 * 
		 */
    private function set_trackChannel(ch : Int) : Int
    {
        for (item/* AS3HX WARNING could not determine type for var: item exp: EIdent(_msgList) type: MessageList */ in _msgList)
        {
    ...
    private function set_trackPatch(ph : Int) : Int
    {
        for (item/* AS3HX WARNING could not determine type for var: item exp: EIdent(_msgList) type: MessageList */ in _msgList)
        {
            if ((Std.is(item, ChannelItem)) && (item.command == MidiEnum.PROGRAM_CHANGE))
